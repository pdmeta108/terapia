...
if( is_object($EM_Event) && !$EM_Event->can_manage('edit_events','edit_others_events') ){
	?>
	<div class="wrap"><h2><?php esc_html_e('Acceso no Autorizado','events-manager'); ?></h2><p><?php echo sprintf(__('No tienes los derechos para administrar este %s.','events-manager'),__('Evento','events-manager')); ?></p></div>
	<?php
	return false;
...
<form enctype='multipart/form-data' id="event-form" class="em-event-admin-editor <?php if( $EM_Event->is_recurring() ) echo 'em-event-admin-recurring' ?>" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null, 'action'=>null))); ?>">
	...
	<div class="wrap">
		...
		<?php if(get_option('dbem_events_anonymous_submissions') && !is_user_logged_in()): ?>
			<h3 class="event-form-submitter"><?php esc_html_e( 'Tus Detalles', 'events-manager'); ?></h3>
			<div class="inside event-form-submitter">
				<div class="form-group">
					<label><?php esc_html_e('Nombre', 'events-manager'); ?></label>
				...
				<div class="form-group">
					<label><?php esc_html_e('Correo', 'events-manager'); ?></label>
		...
		<?php endif; ?>
		<h3 class="event-form-name"><?php esc_html_e( 'Nombre del Evento', 'events-manager'); ?></h3>
		<div class="form-group inside event-form-name">
			...
			<br />
			<?php esc_html_e('El nombre del evento. Por ejemplo: Fiesta de cumpleaños', 'events-manager'); ?>
			<?php em_locate_template('forms/event/group.php',true); ?>
		</div>
					
		<h3 class="event-form-when"><?php esc_html_e( '¿Cuando?', 'events-manager'); ?></h3>
		...
		<h3 class="event-form-where"><?php esc_html_e( '¿Dónde?', 'events-manager'); ?></h3>
		<div class="inside event-form-where">
		<?php em_locate_template('forms/event/location.php',true); ?>
		</div>
		<?php endif; ?>
		
		<h3 class="event-form-details"><?php esc_html_e( 'Detalles', 'events-manager'); ?></h3>
		<div class="inside event-form-details">
			<div class="event-editor">
				...
				<?php else: ?>
					...
					<br />
					<?php esc_html_e( 'Detalles acerca del evento.', 'events-manager')?> <?php esc_html_e( 'Se permite el uso de HTML.', 'events-manager')?>
				<?php endif; ?>
			</div>
		...
		<h3><?php esc_html_e( 'Imagen del Evento', 'events-manager'); ?></h3>
		...
		<!-- START Bookings -->
		<h3><?php esc_html_e('Reservaciones/Registro','events-manager'); ?></h3>
	...
	<div class="submit">
	    ...
	    <input type='submit' class='btn btn-primary submit-btn' value='<?php echo esc_attr(sprintf( __('Crear %s','events-manager'), __('Evento','events-manager') )); ?>' />
	    <?php else: ?>
	    <input type='submit' class='btn btn-primary submit-btn' value='<?php echo esc_attr(sprintf( __('Actualizar %s','events-manager'), __('Evento','events-manager') )); ?>' />
	    <?php endif; ?>
		</div>
