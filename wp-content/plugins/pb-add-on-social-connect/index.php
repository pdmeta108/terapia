...
class WPPB_Social_Connect {
	...
	function wppb_sc_meta_boxes_content( $post ) {
		...
		<div class="wrap">
			<p>
				<label for="wppb_sc_rf_epf_active" ><?php _e( 'Display Social Connect buttons:', 'profile-builder' ) ?></label>
			</p>
			<select name="wppb_sc_rf_epf_active" id="wppb_sc_rf_epf_active" class="mb-select">
				<option value="yes" <?php selected( $wppb_sc_rf_epf_value, 'yes' ); ?>><?php _e( 'Yes', 'profile-builder' ) ?></option>
				<option value="no" <?php selected( $wppb_sc_rf_epf_value, 'no' ); ?>><?php _e( 'No', 'profile-builder' ) ?></option>
			</select>
		</div>
		<?php
	}
            ...
            if ($manage_fields != 'not_set') {
                foreach ($manage_fields as $key => $value) {
                    if ($value['field'] == 'GDPR Checkbox') {
                        $vars_array['gdpr_description'] = esc_attr(trim($value['description']));
	...
	function wppb_sc_handle_platform_id_check() {
		...
		if( ! empty( $_POST['platform_id'] ) ) {
			$result = $wpdb->get_results( $wpdb->prepare( "SELECT meta_value FROM " . $wpdb->base_prefix . "usermeta WHERE meta_key = %s AND meta_value = %s", '_wppb_'. sanitize_text_field( $_POST['platform'] ) .'_connect_id', sanitize_text_field( $_POST['platform_id'] ) ) );
			...
			if( ! empty( $_POST['wppb_sc_security_token'] ) ) {
				...
				if( empty( $user ) && $users_can_register ) {
					...
					if( $email_confirmation_on_off == 'off' ) {
						...
						add_filter( 'wppb_email_confirmation_on_register', 'wppb_sc_disable_email_confirmation' );
                    ...
                    if( array_key_exists( 'wppb_sc_gdpr_checkbox', $_POST ) ? !$_POST['wppb_sc_gdpr_checkbox'] : true ) {
                        $manage_fields = get_option('wppb_manage_fields', 'not_set');
                        if ($manage_fields != 'not_set') {
                            foreach ($manage_fields as $key => $value) {
                                if ($value['field'] == 'GDPR Checkbox') {
                                    die(json_encode(array('action' => 'wppb_sc_gdpr', 'wppb_sc_security_token' => $received_token, 'platform_response' => $_POST['platform_response'])));
                                }
                            }
                        }
                    }

					$user = $this->wppb_sc_register_user($platform, $platform_data, $email, $wppb_general_settings, $wppb_sc_form_ID, array_key_exists( 'wppb_sc_gdpr_checkbox', $_POST ) ? $_POST['wppb_sc_gdpr_checkbox'] : false );
					...
					if( PROFILE_BUILDER == 'Profile Builder Pro' ) {
						...
						if( isset( $wppb_module_settings['wppb_customRedirect'] ) && $wppb_module_settings['wppb_customRedirect'] == 'show' /*&& $_POST['wppb_redirect_priority'] != 'top'*/ && function_exists( 'wppb_custom_redirect_url' ) ) {
							$redirect_url = wppb_custom_redirect_url( 'after_registration', $redirect_to, sanitize_user( $user->data->user_login ), key( $user->caps ) );
	...
	function wppb_sc_register_user( $platform, $platform_data, $email, $wppb_general_settings, $wppb_sc_form_ID, $gdpr = false ) {
		...
		if( defined( 'WPPB_PLUGIN_DIR' ) ) {
			...
			wp_die( __( 'Profile Builder not active!', 'profile-builder' ) );
		}
	...
	function wppb_sc_settings_page_heading_general_settings() {
		echo '<li><h3>' . __( 'General Settings', 'profile-builder' ) . '</h3></li>';
	}

	function wppb_sc_settings_page_heading_application_settings() {
		echo '<li><h3>' . __( 'Application Settings', 'profile-builder' ) . '</h3></li>';
	}

	function wppb_sc_settings_page_heading_appearance_settings() {
		echo '<li><h3>' . __( 'Appearance Settings', 'profile-builder' ) . '</h3></li>';
	}
	...
	function wppb_sc_buttons_for_display_on_forms( $form_ID ) {
		...
		if( isset( $this->wppb_social_connect_settings[0]['unlink-accounts'] ) && $this->wppb_social_connect_settings[0]['unlink-accounts'] == 'yes' ) {
			...
			if( ! empty( $linked_platforms ) ) {
				$linked_accounts_text = __( 'Your account is linked with:', 'profile-builder' );
	...
	function wppb_sc_heading_before_small_buttons() {
		...
		if( $pagenow == 'wp-login.php' ) {
			$heading = '<div class="wppb-sc-heading-before-reg-buttons-wp-default"><h2>' . esc_attr( $this->wppb_social_connect_settings[0]['heading-before-reg-buttons'] ) . '</h2></div>';
		} elseif( isset( $this->forms_type ) && $this->forms_type == 'edit_profile' ) {
			$heading = '<div class="wppb-sc-heading-before-reg-buttons"><h3>' . esc_attr( $this->wppb_social_connect_settings[0]['heading-before-ep-buttons'] ) . '</h3></div>';
		} else {
			$heading = '<div class="wppb-sc-heading-before-reg-buttons"><h3>' . esc_attr( $this->wppb_social_connect_settings[0]['heading-before-reg-buttons'] ) . '</h3></div>';
		}
...
*
*/
function wppb_sc_update_checker(){
    if( class_exists( 'wppb_PluginUpdateChecker' ) ) {
        ...
        $wppb_sc_update = new wppb_PluginUpdateChecker('http://updatemetadata.cozmoslabs.com/?localSerialNumber=' . $localSerial . '&uniqueproduct=CLPBSC', __FILE__, 'wppb-social-connect-add-on');
    }
}
add_action( 'plugins_loaded', 'wppb_sc_update_checker', 100 );
