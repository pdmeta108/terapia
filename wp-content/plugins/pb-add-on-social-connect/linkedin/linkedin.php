<?php
add_filter( 'wppb_sc_process_linkedin_response', 'wppb_sc_linkedin_response' );
function wppb_sc_linkedin_response( $platform_response ) {
...
function wppb_sc_generate_linkedin_button( $form_ID ) {
    ...
    if( ! empty( $social_connect_instance->wppb_social_connect_settings[0]['buttons-style'] ) && $social_connect_instance->wppb_social_connect_settings[0]['buttons-style'] == 'text' ) {
        if( isset( $social_connect_instance->forms_type ) && $social_connect_instance->forms_type == 'edit_profile' ) {
            if( ! empty( $social_connect_instance->wppb_social_connect_settings[0]['linkedin-button-text-ep'] ) ) {
                $button .= esc_attr( $social_connect_instance->wppb_social_connect_settings[0]['linkedin-button-text-ep'] );
            } else {
            ...
            if( ! empty( $social_connect_instance->wppb_social_connect_settings[0]['linkedin-button-text'] ) ) {
                $button .= esc_attr( $social_connect_instance->wppb_social_connect_settings[0]['linkedin-button-text'] );
            } else {
...
add_action( 'wp_head', 'wppb_sc_listen_for_linkeind_login' );
function wppb_sc_listen_for_linkeind_login(){
    global $social_connect_instance;
                            ...
                            if(is_object($profile_info_body) && isset($profile_info_body->id) && $profile_info_body->id && is_object($email_body) && isset($email_body->elements)){
                                ...
                                <script>
                                    var data = {
                                        ...
                                        'wppb_sc_security_token'    : '<?php echo $social_connect_instance->wppb_social_connect_settings[0]['linkedin-client-id']; ?>',//the security token that is set from js is the client id
