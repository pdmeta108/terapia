...
if( ! empty( $wppb_social_connect_settings[0]['twitter-api-key'] ) && ! empty( $wppb_social_connect_settings[0]['twitter-api-secret'] ) ) {
    ...
    function wppb_sc_twitter_oauth_response() {
        $connection = new WPPB_TwitterOAuth( WPPB_SC_TWITTER_CONSUMER_KEY, WPPB_SC_TWITTER_CONSUMER_SECRET );
        ...
        if( $request_token ) {
            ...
            switch( $connection->http_code ) {
                ...
                default:
                    ...
                    wp_die( __( "Connection with twitter Failed", 'profile-builder' ) );
                    break;
            }

        } else { //error receiving request token
            wp_die( __( "Error Receiving Request Token", 'profile-builder' ) );
        }
        wp_die();
    }
    ...
    function wppb_handle_oauth_callback() {
        if( isset( $_GET['oauth_token'] ) ) {
            ...
            $connection = new WPPB_TwitterOAuth( WPPB_SC_TWITTER_CONSUMER_KEY, WPPB_SC_TWITTER_CONSUMER_SECRET, $wppb_sc_security_token->request_token, $wppb_sc_security_token->oauth_token_secret );

			$access_token = $connection->getAccessToken( $_REQUEST['oauth_verifier'] );

			if( $access_token ) {
                $connection = new WPPB_TwitterOAuth( WPPB_SC_TWITTER_CONSUMER_KEY, WPPB_SC_TWITTER_CONSUMER_SECRET, $access_token['oauth_token'], $access_token['oauth_token_secret'] );
				$params = array();
            ...
            echo "<h4>" . __( 'Something went wrong. Please try again later!', 'profile-builder' ) . "</h4>";
        }
    }
}
    ...
    if( ! empty( $social_connect_instance->wppb_social_connect_settings[0]['buttons-style'] ) && $social_connect_instance->wppb_social_connect_settings[0]['buttons-style'] == 'text' ) {
		if( isset( $social_connect_instance->forms_type ) && $social_connect_instance->forms_type == 'edit_profile' ) {
			if( ! empty( $social_connect_instance->wppb_social_connect_settings[0]['twitter-button-text-ep'] ) ) {
				$button .= esc_attr( $social_connect_instance->wppb_social_connect_settings[0]['twitter-button-text-ep'] );
			} else {
			...
			if( ! empty( $social_connect_instance->wppb_social_connect_settings[0]['twitter-button-text'] ) ) {
				$button .= esc_attr( $social_connect_instance->wppb_social_connect_settings[0]['twitter-button-text'] );
			} else {
