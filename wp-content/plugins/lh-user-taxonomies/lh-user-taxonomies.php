...
if ( ( is_string( $object_type) and ( 'user' === $object_type ) ) or ( is_array( $object_type ) and in_array( 'user', $object_type, true ) ) ) {
		...
		foreach(self::$taxonomies as $key=>$taxonomy){
			...
			?>
				...
				<table class="form-table">
					<tr>
						<th><label for=""><?php _e("Seleccione su {$taxonomy->labels->singular_name}")?></label></th>
						<td>
							<?php if(!empty($terms)): ?>
...
<input type="hidden" id="<?php echo self::return_plugin_namespace().'-'.$key.'-backend_edit'; ?>" name="<?php echo self::return_plugin_namespace().'-'.$key.'-backend_edit'; ?>" value="1" />
		...
		$output	= ob_get_clean();
		if(!empty($output)) {
			echo '<h3>', __('Profesiones', self::return_plugin_namespace()), '</h3>';
			echo $output;
		}
	}
