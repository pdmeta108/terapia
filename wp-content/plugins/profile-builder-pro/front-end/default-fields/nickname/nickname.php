...
function wppb_nickname_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	...
	if ( $form_location != 'back_end' ){
        ...
        $output = '
			<input class="form-control floating text-input default_field_nickname '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="nickname" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70, $field ) .'" type="text" id="nickname" value="'. esc_attr( wp_unslash( $input_value ) ) .'" '. $extra_attr .'/>
			<label class="focus-label" for="nickname">'.$item_title.$error_mark.'</label>';
