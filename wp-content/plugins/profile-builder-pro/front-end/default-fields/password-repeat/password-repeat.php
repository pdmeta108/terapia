...
function wppb_password_repeat_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	...
	if ( $form_location != 'back_end' ){
        ...
        $output = '
			<input class="form-control floating text-input '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="passw2" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70, $field ) .'" type="password" id="passw2" value="" autocomplete="off" '. $extra_attr .'/>
			<label class="focus-label" for="passw2">' . $item_title.$error_mark . '</label>';
