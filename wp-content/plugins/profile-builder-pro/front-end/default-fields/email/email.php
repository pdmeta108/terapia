...
function wppb_email_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
        ...
        $output = '
			<input class="text-input default_field_email form-control floating'. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="email" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70, $field ) .'" type="email" id="email" value="'. esc_attr( $input_value ) .'" '. $extra_attr .' />
            <label class="focus-label" for="email">'.$item_title.$error_mark.'</label>';
