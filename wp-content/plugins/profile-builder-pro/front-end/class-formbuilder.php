...
class Profile_Builder_Form_Creator{
	...
	function wppb_form_content( $message ) {
        ...
        <form enctype="multipart/form-data" method="post" id="<?php echo apply_filters( 'wppb_form_id', $wppb_form_id, $this ); ?>" class="<?php echo apply_filters( 'wppb_form_class', $wppb_form_class, $this ); ?>" action="<?php echo esc_url( apply_filters( 'wppb_form_action', wppb_curpageurl() ) ); ?>">
			...
			echo apply_filters( 'wppb_before_form_fields', '', $this->args['form_type'], $this->args['ID'] );
			echo $this->wppb_output_form_fields( $_REQUEST, $field_check_errors, $this->args['form_fields'] );
			echo apply_filters( 'wppb_after_form_fields', '', $this->args['form_type'], $this->args['ID'] );

			echo apply_filters( 'wppb_before_send_credentials_checkbox', '', $this->args['form_type'], $this->args['ID'] );
			$this->wppb_add_send_credentials_checkbox( $_REQUEST, $this->args['form_type'] );
			echo apply_filters( 'wppb_after_send_credentials_checkbox', '', $this->args['form_type'] );
			...
			<p class="form-submit" <?php echo $wppb_form_submit_extra_attr; ?> >
				...
				<input name="<?php echo $this->args['form_type']; ?>" type="submit" id="<?php echo $this->args['form_type']; ?>" class="btn btn-primary btn-block btn-lg login-btn <?php echo apply_filters( 'wppb_'. $this->args['form_type'] .'_submit_class', "submit button" );?>" value="<?php echo apply_filters( 'wppb_'. $this->args['form_type'] .'_button_name', esc_attr( $button_name ), $this->args['form_name'] ); ?>" <?php echo apply_filters( 'wppb_form_submit_button_extra_attributes', '', $this->args['form_type'] );?>/>
	...
	function wppb_output_form_fields( $global_request, $field_check_errors, $form_fields, $called_from = NULL ){
		...
		if( !empty( $form_fields ) ){
			...
			foreach( $form_fields as $field ){
				...
				$output_fields .= apply_filters( 'wppb_output_before_form_field', '<div class="form-group form-focus '. $css_class .'" id="wppb-form-element-'. $field['id'] .'">', $field, $error_var, $this->args['role'] );
				...
				$output_fields .= apply_filters( 'wppb_output_after_form_field', '</div>', $field, $this->args['ID'], $this->args['form_type'], $called_from );
			}
