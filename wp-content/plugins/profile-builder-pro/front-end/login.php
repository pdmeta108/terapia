	...
	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="'. esc_url( wppb_curpageurl() ) .'" method="post">
			...
			<div class="form-group form-focus">
				<input type="text" name="log" id="' . esc_attr( $args['id_username'] ) . '" class="input form-control floating" value="' . esc_attr( $args['value_username'] ) . '" size="20" />
				<label class="focus-label" for="' . esc_attr( $args['id_username'] ) . '">' . esc_html( $args['label_username'] ) . '</label>
				</div>
				<div class="form-group form-focus">
				<input type="password" name="pwd" id="' . esc_attr( $args['id_password'] ) . '" class="input form-control floating" value="" size="20" />
				<label class="focus-label" for="' . esc_attr( $args['id_password'] ) . '">' . esc_html( $args['label_password'] ) . '</label>
			</div>
			' . $login_form_middle . '
			' . ( $args['remember'] ? '<p class="login-remember"><label><input name="rememberme" type="checkbox" id="' . esc_attr( $args['id_remember'] ) . '" value="forever"' . ( $args['value_remember'] ? ' checked="checked"' : '' ) . ' /> ' . esc_html( $args['label_remember'] ) . '</label></p>' : '' ) . '
			<p class="login-submit">
				<input type="submit" name="wp-submit" id="' . esc_attr( $args['id_submit'] ) . '" class="'. apply_filters( 'wppb_login_submit_class', "button button-primary" ) . ' btn btn-primary btn-block btn-lg login-btn" value="' . esc_attr( $args['label_log_in'] ) . '" />
