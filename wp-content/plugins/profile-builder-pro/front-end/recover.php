 ...
 */
 function wppb_create_generate_password_form( $post_data ){
	...
	if( !empty( $wppb_generalSettings['loginWith'] ) && $wppb_generalSettings['loginWith'] == 'email' ){
		$recover_notification = '<p class="small text-muted">' . __( 'Please enter your email address.', 'profile-builder' );
		$username_email_label = __( 'Email', 'profile-builder' );
	}
	else{
		$recover_notification = '<p class="small text-muted">' . __( 'Please enter your username or email address.', 'profile-builder' );
	...
	$recover_input = '<div class="form-group form-focus">
                <input class="form-control floating" name="username_email" type="text" id="username_email" value="'.esc_attr( trim( $username_email ) ).'" '. apply_filters( 'wppb_recover_password_extra_attr', '', $username_email_label, 'username_email' ) .' />
	            <label class="focus-label" for="username_email">'. $username_email_label .'</label>
			</div>';
	echo apply_filters( 'wppb_recover_password_generate_password_input', $recover_input, trim( $username_email ) );
		...
		<input name="recover_password" type="submit" id="wppb-recover-password-button" class="<?php echo apply_filters( 'wppb_recover_submit_class', "submit button" );?> btn btn-primary btn-block btn-lg login-btn" value="<?php echo apply_filters('wppb_recover_password_button_name3', $button_name); ?>" />
