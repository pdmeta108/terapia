...
function wppb_userlisting_more_info( $value, $name, $children, $extra_info ){
	...
	if ( apply_filters( 'wbb_userlisting_extra_more_info_link_type', true ) )
		return apply_filters( 'wppb_userlisting_more_info_link', '<span id="wppb-more-span" class="wppb-more-span"><a href="'.$more_url.'" class="wppb-more btn btn-sm bg-info-light" id="wppb-more" title="'.__( 'Click here to see more information about this user', 'profile-builder' ) .'" alt="'.__( 'More...', 'profile-builder' ).'"><i class="far fa-eye"></i> '.__( 'More...', 'profile-builder').'</a></span>', $more_url );
...
function wppb_userlisting_go_back_link( $value, $name, $children, $extra_values ){
	if ( apply_filters( 'wppb_userlisting_go_back_link_type', true ) )
		return apply_filters( 'wppb_userlisting_go_back_link', '<div id="wppb-back-span" class="wppb-back-span"><a href=\'javascript:history.go(-1)\' class="single-user-button btn btn-sm bg-danger-light wppb-back" id="wppb-back" title="'. __( 'Click here to go back', 'profile-builder' ) .'" alt="'. __( 'Back', 'profile-builder' ) .'">'. __( 'Back', 'profile-builder' ) .'</a></div>' );
...
function wppb_userlisting_extra_search_all_fields( $value, $name, $children, $extra_info ){
	...
	return '
		<form method="post" action="'.esc_url( add_query_arg( array( 'wppb_page' => 1, 'setSortingCriteria' => $setSortingCriteria, 'setSortingOrder' => $setSortingOrder ), wppb_remove_query_arg( 'wppb_page', wppb_curpageurl() ) ) ).'" class="wppb-search-users wppb-user-forms">
            <div class="form-group search-info wppb-search-users-wrap">
                <input class="form-control"  type="text" id="wppb-search-fields" name="searchFor" title="'. $searchText .'" placeholder="'.$searchText.'" />
		        <input type="hidden" name="action" value="searchAllFields" />
		    </div>
            <input type="submit" name="searchButton" class="btn btn-primary search-btn wppb-search-button" value="'.__( 'Search', 'profile-builder' ).'" />
